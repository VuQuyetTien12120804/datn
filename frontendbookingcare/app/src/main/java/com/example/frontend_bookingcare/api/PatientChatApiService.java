...
public interface PatientChatApiService {

    @GET("api/v1/patient/messages/threads")
    Call<ApiEnvelope> listThreads(@Header("Authorization") String authorization);

    @GET("api/v1/patient/messages/threads/{threadKey}/messages")
    ...
    @POST("api/v1/patient/messages/threads/{threadKey}/messages")
    Call<ApiEnvelope> sendMessage(@Header("Authorization") String authorization,
                                  @Path(value = "threadKey", encoded = true) String threadKey,
                                  @Body SendChatMessageRequest body);

    @POST("api/v1/patient/messages/threads/{threadKey}/read")

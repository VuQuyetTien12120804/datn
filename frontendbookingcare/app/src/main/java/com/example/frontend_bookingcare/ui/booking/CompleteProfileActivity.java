...
public class CompleteProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        dob.setText(ex.dob);
        gender.setText(BookingFormatters.displayGender(this, ex.gender));
        address.setText(ex.address);

        gender.setAdapter(new ArrayAdapter<>(this,
                android.R.layout.simple_list_item_1, BookingFormatters.genderLabels(this)));
        ...
        save.setOnClickListener(v -> {
            ...
            if (TextUtils.isEmpty(p) || TextUtils.isEmpty(d) || TextUtils.isEmpty(g) || TextUtils.isEmpty(a)) {
                Toast.makeText(this, R.string.complete_profile_required, Toast.LENGTH_SHORT).show();
                return;
            }
            if (!BookingFormatters.isValidVnPhone(p)) {
                Toast.makeText(this, R.string.profile_phone_invalid, Toast.LENGTH_SHORT).show();
                return;
            }
            if (!BookingFormatters.isValidDobDdMmYyyy(d)) {
                Toast.makeText(this, R.string.profile_dob_invalid, Toast.LENGTH_SHORT).show();
                return;
            }
            String genderEnum = BookingFormatters.genderToEnum(g);
            if (!sm.isLoggedIn() || sm.getSession() == null || TextUtils.isEmpty(sm.getSession().accessToken)) {
                Toast.makeText(this, R.string.booking_need_login, Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            new PatientProfileRepository().updateMe(
                    bearer,
                    new UpdatePatientProfileRequest(p, d, genderEnum, a),
                    new PatientProfileRepository.RepoCallback() {
                        @Override
                        public void onSuccess(com.example.frontend_bookingcare.api.ApiEnvelope env) {
                            runOnUiThread(() -> {
                                sm.saveProfileExtras(new ProfileExtras(p, d, genderEnum, a));
                                Toast.makeText(CompleteProfileActivity.this, R.string.complete_profile_saved, Toast.LENGTH_SHORT).show();
                                setResult(RESULT_OK);
                                finish();
                            });
                        }
